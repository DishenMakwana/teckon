...
import { Calendar, User, BookOpen, Mail, Phone, MessageSquare } from "lucide-react";
    ...
    <>
      ...
      <section className="bg-teckon-dark-blue py-20 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <h1 className="text-3xl sm:text-5xl font-black text-white mt-6 mb-6 leading-tight max-w-4xl">{post.title}</h1>
          ...
          <div className="flex flex-wrap items-center gap-y-3 gap-x-6 text-white/80 text-sm font-mono">
            ...
            <span className="flex items-center gap-1.5">
              <BookOpen className="h-4 w-4 text-[#FFBE00]" />
              5 Min Read
            </span>
          </div>
        </div>
              ...
              <div className="space-y-6 text-slate-600 font-medium leading-relaxed">
                {post.content.split("\n").map((line, i) => {
                  ...
                  if (trimmed.startsWith("## ")) {
                    return (
                      <h2 key={i} className="text-2xl font-black text-[#0B0F19] mt-8 mb-4 border-b border-slate-100 pb-2 uppercase font-mono tracking-wide">
                        {trimmed.slice(3)}
                      </h2>
                    );
                  }
                  ...
                  if (trimmed.startsWith("**") && trimmed.endsWith("**")) {
                    return (
                      <p key={i} className="font-extrabold text-[#0B0F19] my-3 text-base">
                        {trimmed.slice(2, -2)}
                      </p>
                    );
                  }
                  ...
                  if (trimmed.startsWith("- ")) {
                    return (
                      <li key={i} className="flex items-start gap-2 text-slate-600 text-sm ml-2">
            ...
            <aside className="lg:col-span-1 lg:sticky lg:top-24 self-start space-y-6">
              ...
              <div className="bg-[#0B0F19] rounded-3xl p-6 border border-white/5 shadow-2xl relative overflow-hidden text-center md:text-left">
                ...
                <div className="relative z-10 space-y-4">
                  ...
                  <h3 className="text-white font-black text-lg leading-tight">Need Hydraulic Parts?</h3>
                  <p className="text-slate-400 text-xs leading-relaxed">Get a quick quote on JCB, Hitachi, Terex, and Case replacement components direct from our factory floors in Rajkot.</p>
                  <Link
                    href="/contact"
              ...
              <div className="bg-gray-50 rounded-3xl p-6 border border-gray-100 shadow-sm space-y-4">
                ...
                <div className="flex flex-col gap-3 text-xs font-semibold">
                  <a href="tel:+916351879842" className="flex items-center gap-2.5 text-teckon-blue hover:text-[#FF6B35] transition-colors">
                    <span className="p-1.5 bg-slate-200/50 rounded-lg text-slate-600"><Phone className="h-3.5 w-3.5" /></span>
                    <span>+91-63518 79842</span>
                  </a>
                  <a href="mailto:shreejihyd4008@gmail.com" className="flex items-center gap-2.5 text-teckon-blue hover:text-[#FF6B35] transition-colors break-all">
                    <span className="p-1.5 bg-slate-200/50 rounded-lg text-slate-600"><Mail className="h-3.5 w-3.5" /></span>
                    <span>shreejihyd4008@gmail.com</span>
                  </a>
                  <a href="https://wa.me/919426915578" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2.5 text-emerald-600 hover:text-emerald-700 transition-colors">
                    <span className="p-1.5 bg-emerald-100/50 text-emerald-600 rounded-lg"><MessageSquare className="h-3.5 w-3.5" /></span>
                ...
                {related.map((r) => (
                  ...
                  >
                    <div className="relative w-24 h-24 rounded-xl overflow-hidden shrink-0 bg-slate-100">
                      <SafeImage src={r.image} alt={r.title} fill sizes="96px" className="object-cover group-hover:scale-105 transition-transform duration-500" />
                    </div>
                    <div className="flex flex-col justify-center min-w-0">
                      <span className="text-[10px] text-[#FF6B35] font-mono font-bold uppercase tracking-wider">{r.category}</span>
                      <h3 className="font-extrabold text-[#0B0F19] text-sm leading-snug mt-1 group-hover:text-[#FF6B35] transition-colors line-clamp-2">{r.title}</h3>
                    </div>
                  </Link>
                ))}
              </div>
